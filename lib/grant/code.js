...
module.exports = function code(options, issue, extend) {
  ...
  function request(req) {
    ...
    if (typeof clientID !== 'string') {
      throw new AuthorizationError(
        'Invalid parameter: client_id must be a string',
        'invalid_request'
      );
    }
    ...
    if (scope) {
      if (typeof scope !== 'string') {
        throw new AuthorizationError(
          'Invalid parameter: scope must be a string',
          'invalid_request'
        );
      }
  ...
  function response(txn, res, complete, next) {
    ...
    function issued(err, code) {
      ...
      if (!code) {
        return next(
          new AuthorizationError('Request denied by authorization server', 'access_denied')
        );
      }
