...
module.exports = function (server, options, validate, immediate, complete) {
  ...
  return function authorization(req, res, next) {
    ...
    server._parse(type, req, function (err, areq) {
      ...
      if (!areq || !areq.type) {
        return next(
          new AuthorizationError('Missing required parameter: response_type', 'invalid_request')
        );
      }
      if (areq.type && !areq.clientID) {
        return next(
          new AuthorizationError('Unsupported response type: ' + type, 'unsupported_response_type')
        );
      }
      ...
      function validated(err, client, redirectURI, webOrigin) {
        ...
        if (webOrigin) {
          req.oauth2.webOrigin = webOrigin;
        }
        req.oauth2.req = areq;
        req.oauth2.user = req[userProperty];
        if (req.locals) {
          req.oauth2.locals = req.locals;
        }
        ...
        function immediated(err, allow, info, locals) {
          ...
          if (allow) {
            ...
            if (locals) {
              ...
              Object.assign(req.oauth2.locals, locals);
            }
