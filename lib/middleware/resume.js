...
module.exports = function (server, options, immediate, complete) {
  ...
  if (!server) {
    throw new TypeError('oauth2orize.resume middleware requires a server argument');
  }
  if (!immediate) {
    throw new TypeError('oauth2orize.resume middleware requires an immediate function');
  }
  ...
  return function resume(req, res, next) {
    ...
    if (res.locals) {
      ...
      Object.assign(req.oauth2.locals, res.locals);
    }
