...
describe('exchange.authorizationCode', function () {
  ...
  describe('issuing an access token', function () {
    ...
    before(function (_, done) {
      function issue(client, code, redirectURI, done) {
        ...
        if (code !== 'abc123') {
          return done(new Error('incorrect code argument'));
        }
        if (redirectURI !== 'http://example.com/oa/callback') {
  ...
  describe('issuing an access token and refresh token', function () {
    ...
    it('should respond with body', function () {
      expect(response.body).to.equal(
        '{"access_token":"s3cr1t","refresh_token":"getANotehr","token_type":"Bearer"}'
      );
    });
  });
  ...
  describe('issuing an access token and params', function () {
    ...
    it('should respond with body', function () {
      expect(response.body).to.equal(
        '{"access_token":"s3cr1t","expires_in":3600,"token_type":"Bearer"}'
      );
    });
  });
    ...
    before(function (_, done) {
      chai.connect
        ...
        .req(function (req) {
          ...
          req.body = {
            code: 'abc123',
            redirect_uri: 'http://example.com/oa/callback',
            code_verifier: 's3cr1t'
          };
        })
        .end(function (res) {
          response = res;
  ...
  describe('issuing an access token based on authInfo', function () {
    ...
    function issue(client, code, redirectURI, body, authInfo, done) {
      ...
      if (authInfo.ip !== '127.0.0.1') {
        return done(new Error('incorrect authInfo argument'));
      }
  ...
  describe('with user property option issuing an access token', function () {
    ...
    before(function (_, done) {
      ...
      chai.connect
        .use(authorizationCode({ userProperty: 'client' }, issue))
        .req(function (req) {
