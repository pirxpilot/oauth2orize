...
describe('authorization', function () {
  describe('immediate response', function () {
    ...
    describe('based on client and user', function () {
      ...
      before(function (_, done) {
        chai.connect
          .use('express', authorization(server, validate, immediate))
          .req(function (req) {
            request = req;
            req.query = {
              response_type: 'code',
    ...
    describe('based on client, user, scope, type, and authorization request', function () {
      ...
      before(function () {
        immediate = function (client, user, scope, type, areq, done) {
          ...
          if (areq.audience !== 'https://api.example.com/') {
    ...
    describe('based on complete transaction', function () {
      ...
      before(function () {
        immediate = function (txn, done) {
          ...
          if (txn.req.scope !== 'profile') {
            return done(new Error('incorrect scope argument'));
          }
          if (txn.req.type !== 'code') {
            return done(new Error('incorrect type argument'));
          }
          if (txn.req.audience !== 'https://api.example.com/') {
            return done(new Error('incorrect areq argument'));
          }
          if (txn.locals.ip !== '123.45.67.890') {
            return done(new Error('incorrect locals argument'));
          }
    ...
    describe('encountering an error', function () {
      ...
      before(function (_, done) {
        chai.connect
          .use(authorization(server, validate, immediate))
          .req(function (req) {
            request = req;
            req.query = {
              response_type: 'code',
    ...
    describe('using non-legacy transaction store', function () {
      ...
      describe('based on client, user, and scope, with result that supplies info and locals', function () {
        ...
        it('should serialize transaction', function () {
          ...
          expect(request.__mock_store__.txn.redirectURI).to.equal(
            'http://example.com/auth/callback'
          );
  ...
  describe('immediate response with complete callback', function () {
    ...
    describe('based on transaction', function () {
      ...
      before(function (_, done) {
        chai.connect
          .use('express', authorization(server, validate, immediate, complete))
          .req(function (req) {
            request = req;
            req.query = {
              response_type: 'code',
