  ...
  describe('request parsing', function () {
    ...
    describe('handling a request for authorization with empty query', function () {
      ...
      before(function (_, done) {
        chai.connect
          .use(authorization(server, validate))
          .req(function (req) {
            request = req;
            req.query = {};
            req.session = {};
  ...
  describe('using legacy transaction store', function () {
    ...
    before(function () {
      validate = function (clientID, redirectURI, done) {
        ...
        if (redirectURI !== 'http://example.com/auth/callback') {
          return done(new Error('incorrect redirectURI argument'));
        }
    ...
    describe('handling a request for authorization', function () {
      ...
      it('should store transaction in session', function () {
        ...
        expect(request.session['authorize'][tid].redirectURI).to.equal(
        ...
        expect(request.session['authorize'][tid].req.redirectURI).to.equal(
          'http://example.com/auth/callback'
        );
      });
    });
    ...
    describe('handling a request for authorization, validating with scope', function () {
      ...
      before(function () {
        validate = function (clientID, redirectURI, scope, done) {
          ...
          if (scope !== 'write') {
            return done(new Error('incorrect scope argument'));
          }
    ...
    describe('handling a request for authorization, validating with scope and type', function () {
      ...
      before(function () {
        validate = function (clientID, redirectURI, scope, type, done) {
          ...
          if (type !== 'code') {
            return done(new Error('incorrect type argument'));
          }
    ...
    describe('handling a request for authorization, validating with authorization request', function () {
      ...
      before(function () {
        validate = function (areq, done) {
          if (areq.clientID !== '1234') {
            return done(new Error('incorrect client argument'));
          }
          if (areq.redirectURI !== 'http://example.com/auth/callback') {
            return done(new Error('incorrect redirectURI argument'));
          }
    ...
    describe('with id length option', function () {
      describe('handling a request for authorization', function () {
        ...
        before(function (_, done) {
          chai.connect
            .use(authorization(server, { idLength: 12 }, validate))
            .req(function (req) {
              request = req;
              req.query = {
                response_type: 'code',
    ...
    describe('with session key option', function () {
      describe('handling a request for authorization', function () {
        ...
        before(function (_, done) {
          chai.connect
            .use(authorization(server, { sessionKey: 'oauth2z' }, validate))
            .req(function (req) {
              request = req;
              req.query = {
                response_type: 'code',
  ...
  describe('using non-legacy transaction store', function () {
    ...
    describe('handling a request for authorization', function () {
      ...
      it('should serialize transaction', function () {
        ...
        expect(request.__mock_store__.txn.req.redirectURI).to.equal(
          'http://example.com/auth/callback'
        );
