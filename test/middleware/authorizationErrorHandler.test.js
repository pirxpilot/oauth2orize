...
describe('authorizationErrorHandler', function () {
  ...
  it('should throw if constructed without a server argument', function () {
    expect(function () {
      ...
      TypeError,
      'oauth2orize.authorizationErrorHandler middleware requires a server argument'
    );
  });
  ...
  describe('using legacy transaction store', function () {
    ...
    describe('handling an error', function () {
      ...
      before(function (_, done) {
        chai.connect
          .use('express', authorizationErrorHandler(server))
          .req(function (req) {
            request = req;
            req.query = {};
            req.body = {};
      ...
      it('should respond', function () {
        ...
        expect(response.getHeader('Location')).to.equal(
          'http://example.com/auth/callback?error_description=something went wrong'
        );
      });
