  ...
  describe('using legacy transaction store', function () {
    ...
    before(function () {
      ...
      server.grant('code', 'response', function (txn, res, next) {
        if (txn.transactionID !== 'abc123') {
          return next(new Error('incorrect transaction argument'));
        }

        if (txn.res.allow === false) {
          return res.redirect(txn.redirectURI + '?error=access_denied');
        }
    ...
    describe('handling a user decision to allow access', function () {
      ...
      it('should respond', function () {
        ...
        expect(response.getHeader('Location')).to.equal(
          'http://example.com/auth/callback?code=a1b1c1'
        );
      });
    ...
    describe('with cancel field option', function () {
      describe('handling a user decision to deny access', function () {
        ...
        before(function (_, done) {
          chai.connect
            .use('express', decision(server, { cancelField: 'deny' }))
            .req(function (req) {
              request = req;
              req.query = {};
              req.body = { deny: 'Deny' };
    ...
    describe('with session key option', function () {
      describe('handling a user decision to allow access', function () {
        ...
        before(function (_, done) {
          chai.connect
            .use('express', decision(server, { sessionKey: 'oauth2orize' }))
            .req(function (req) {
              request = req;
              req.query = {};
              req.body = {};
    ...
    describe('with user property option', function () {
      describe('handling a user decision to allow access', function () {
        ...
        before(function (_, done) {
          chai.connect
            .use('express', decision(server, { userProperty: 'other' }))
            .req(function (req) {
              request = req;
              req.query = {};
              req.body = {};
